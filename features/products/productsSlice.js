  ...
  endpoints: (builder) => ({
    ...
    getTopProducts: builder.query({
      ...
      transformResponse: (responseData) => {
        ...
        return productsAdapter.setAll(initialState, productsArray);
      },
        ...
        const patchResultById = dispatch(
          productsSlice.util.updateQueryData(
            ...
            (draft) => {
              ...
              draft.rating =
                draft.reviews.reduce((sum, r) => sum + r.rating, 0) /
...
export const selectBestSellers = (state) => {
  const topProductsResult =
    productsSlice.endpoints.getTopProducts.select()(state);
  return (
    topProductsResult?.data?.ids?.map(
      (id) => topProductsResult.data.entities[id]
    ) || []
  );
};
...
export const selectTodayDeals = (state) => {
  ...
  return (
    dealsResult?.data?.ids?.map((id) => dealsResult.data.entities[id]) || []
  );
};
...
export const selectNewArrivals = (state) => {
  const newProductsResult =
    productsSlice.endpoints.getNewProducts.select()(state);
  return (
    newProductsResult?.data?.ids?.map(
      (id) => newProductsResult.data.entities[id]
    ) || []
  );
};
...
export const selectRelatedProducts = (productId) =>
  createSelector(
    [
      (state) =>
    ...
    (result) =>
      result.data ? result.data.ids.map((id) => result.data.entities[id]) : []
  );

// Select similar products for a given product ID
export const selectSimilarProducts = (productId) =>
  createSelector(
    [
      (state) =>
...
export const selectProductLoading = (productId) =>
  createSelector(
    [
      (state) =>
...
export const selectProductError = (productId) =>
  createSelector(
    [
      (state) =>
        productsSlice.endpoints.getProductById.select(productId)(state),
